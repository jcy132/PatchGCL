...
class SCCCycleGANModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_SCC', type=float, default=1.0, help='weight for GAN loss (B -> A)')
            ...
            parser.add_argument('--lambda_SRC', type=float, default=1.0, help='weight for SRC')
            ...
            parser.add_argument('--num_knn', type=int, default=30)
            parser.add_argument('--pooling_num', type=int, default=1)
            parser.add_argument('--down_scale', type=int, default=8)
            parser.add_argument('--pooling_ratio', type=str, default='1, 1, 1',
                                help='Ratio for pooling level | [ level0, level1, level2 ]')
            parser.add_argument('--SApool', action='store_true')
            parser.add_argument('--use_nonzero', action='store_true')
            parser.add_argument('--nonzero_th', type=float, default=0.6)
            parser.add_argument('--weighted_graph', action='store_true')
            parser.add_argument('--sep_mode', default='None', type=str)  # sep_adj | sep_adjGnn | sep_adjGnnPool

            parser.add_argument('--norm', default='instance', type=str)  # sep_adj | sep_adjGnn | sep_adjGnnPool
    ...
    def __init__(self, opt):
        ...
        self.model_names += ['D_A_pixel', 'D_B_pixel']
        ...
        if self.isTrain:
            ...
            # self.netD_A_pixel.to(self.device)
            # self.netD_B_pixel.to(self.device)
